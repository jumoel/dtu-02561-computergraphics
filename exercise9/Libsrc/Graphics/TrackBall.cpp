...
namespace Graphics
{
	...
	TrackBall::TrackBall(const Vec3f& _centre, 
											 ...
											 unsigned _width, 
											 unsigned _height):
		centre(_centre), eye_dist(_eye_dist), scale(_scale), 
...
// 	void TrackBall::set_eye_dir(const Vec3f& eye_dir)
// 	{
// 		Vec3f up(0,1,0);
// 		qrot.make_rot(Vec3f(0,0,1), eye_dir);
// 		Vec3f v2 = qrot.apply(Vec3f(0,0,1));
// 		Quaternion q;
// 		q.make_rot(qrot.apply(up),up-dot(v2,up)*v2);
// 		qrot = q*qrot;
//	}
	...
	{
		Vec2f diff = new_v - last_pos;
		trans[0] += diff[0] * scale[0];
		trans[1] += diff[1] * scale[1];
	}

	void TrackBall::zoom(const Vec2f& new_v) 
	{
		trans[2] += (new_v[1] - last_pos[1]) * scale[2];
	}
	...
	void TrackBall::get_view_param(Vec3f& eye, Vec3f& _centre, Vec3f& up) const
	{
		...
		eye = qrot.apply(Vec3f(0,0,1)*(eye_dist + trans[2])) + _centre;
	}
